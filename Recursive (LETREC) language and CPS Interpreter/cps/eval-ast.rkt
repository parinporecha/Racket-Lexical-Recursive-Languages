...
(define eval-ast/k
  (lambda (a e k)
    (cases ast a
      ...
      [boolean (x) (k x)]
      [if-ast (c then else) (eval-ast/k c e (lambda(x) (eval-ast/k (if x then else) e k)))]
      [app (rator rands)
            (eval-ast/k rator e 
	      (lambda(vp)
      ...
      [assume (binds body)
		...
		(map/k
		 ...
		 binds
		 (lambda(vals)
		  (let ([newe (extended-env idss vals e)])
		   (eval-ast/k body newe k))))))]
      [function (formals body) (k (closure formals body e))]
...
(define map/k
 (lambda (pr ls k)
  (cond
   [(null? ls) (k ls)]
   [else (pr (first ls) (lambda(v) (map/k pr (rest ls) (lambda(w) (k (cons v w))))))])))
...
(define (get-bind-id/k b k)
  (cases bind b
	[make-bind (id-sym dval) (k id-sym)]))

(define (bind-ast/k b k)
  (cases bind b
	 [make-bind (id-sym dval) (k dval)]))

(define (get-fbind-fid/k b k)
  (cases fbind b
	 [make-fbind (fid formals body) (k fid)]))

(define (get-fbind-formals/k b k)
  (cases fbind b
	 [make-fbind (fid formals body) (k formals)]))

(define (get-fbind-body/k b k)
  (cases fbind b
	 [make-fbind (fid formals body) (k body)]))
	 ...
	 (midType (list-ref args 0)))
	  ...
	  (lambda(ls)
	   (foldl/k (lambda(x y n) (n (and x y))) #t ls
	    (lambda(bool)
	     (if bool (k (apply op  args)) (error 'apply-prim-op "incorrect number or type of arguments to ~a" op))))))]
